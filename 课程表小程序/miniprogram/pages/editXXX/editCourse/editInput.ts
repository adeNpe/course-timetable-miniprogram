...
Page({
  ...
  onLoad: function (options: { id: any }) {
    ...
    wx.request({
      ...
      success: res => {
        that.setData({
          groupIndex: (res.data as myDataType[])[0].group,
          _res: res.data,
          dayIndex: (res.data as myDataType[])[0].whichDay,
          startTime: (res.data as myDataType[])[0].startTime,
          endTime: (res.data as myDataType[])[0].endTime,
        })
        const rgbColor = hex2rgb((res.data as myDataType[])[0].color);
        if (rgbColor !== null) {
          that.setData({
            color: rgbColor
          })
